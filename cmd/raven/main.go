...
func main() {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "For usage and help, use the --help flag\n")
		os.Exit(1)
	}

	// Check if no flags are provided
	if len(os.Args) == 1 {
		fmt.Println("For usage and help, use the --help flag")
		os.Exit(1)
	}
...
`)
	...
	words, lineCount, err := flags.ReadWordlist(cfg.Wordlist, cfg.Silent)
	...
	if !cfg.Silent {
		...
		if cfg.Wordlist != "" {
			fmt.Printf("[\033[96m*\033[0m] Wordlist: %s (%d lines)\n", cfg.Wordlist, lineCount)
		} else {
			fmt.Printf("[\033[96m*\033[0m] Wordlist: %s (default, %d lines)\n", flags.DefaultWordlistPath, lineCount)
		}
		fmt.Printf("[\033[96m*\033[0m] HTTP Method: %s\n", cfg.Method)
		fmt.Printf("[\033[96m*\033[0m] Threads: %d\n", cfg.Threads)
		fmt.Printf("[\033[96m*\033[0m] Rate Limit: %.1f%s\n", cfg.RateLimit, map[bool]string{true: " (default)", false: ""}[cfg.RateLimit == 100.0])
		if cfg.Proxy != "" {
	...
	results, err := discovery.ScanSubdomains(cfg, words)
	if err != nil {
		fmt.Fprintf(os.Stderr, "[\033[96m*\033[0m] Error: %v\n", err)
		os.Exit(1)
	}
